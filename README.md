...
6. **주간 날씨** — OpenWeatherMap 5일/3시간 예보를 **현지 날짜 기준 일별 최저·최고 기온으로 집계**해 예보 칩으로 표시
7. **인터랙티브 세계지도** — 등거리 투영(equirectangular) 지도 위에 위도/경도를 %좌표로 변환한 **주요 도시 40곳의 점(dot)** 을 배치, 클릭 시 해당 도시 즉시 검색
8. **도시 대표 사진 배경** — Google Places 관광지 사진을 고해상도 전체 배경으로 적용 (크로스페이드 + Ken Burns 효과). **첫 화면은 한국을 상징하는 경복궁 사진**으로 시작
9. **동적 테마** — 날씨 상태(맑음/구름/비/눈/뇌우/안개) × 낮/밤 조합에 따라 **배경 사진 위 색상 틴트와 그라데이션이 부드럽게 전환** (`@property` 로 색상 값 자체를 transition)
10. **Glassmorphism UI** — `backdrop-filter: blur` 기반 유리 질감 카드, 떠다니는 블러 오브, 순차 등장 애니메이션
11. **데모 모드** — API 키가 없으면 샘플 데이터로 전체 UI가 자동 동작 (키 발급 전 확인/시연용 안전망)
